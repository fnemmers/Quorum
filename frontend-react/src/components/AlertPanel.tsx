...
export default function AlertPanel() {
  ...
  return (
    <div className="bg-panel border border-border rounded p-3 space-y-3">
      <div className="text-xs text-gray-500 uppercase tracking-widest">Price Alerts</div>

      <div className="space-y-1 max-h-28 overflow-y-auto">
        {alerts.length === 0 && <div className="text-gray-600 text-xs">No alerts set</div>}
        {alerts.map((a) => (
          <div key={a.id} className="flex items-center justify-between text-xs font-mono">
            <span className="text-accent w-12">{a.symbol}</span>
            <span className={a.condition === 'above' ? 'text-bull' : 'text-bear'}>
              {a.condition} ${a.price.toFixed(2)}
            </span>
            <button onClick={() => removeAlert(a.id)} className="text-gray-600 hover:text-bear">✕</button>
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-border pt-2 grid grid-cols-3 gap-1">
        ...
        <select
          ...
          className="bg-surface border border-border text-white text-xs px-2 py-1 rounded focus:outline-none"
        >
